...
function zeus_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'zeus' ),
        'next_string'     => esc_html__( '&raquo;', 'zeus' ),
        'before_output'   => '<div class="zeus-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'zeus_pagination_defaults', $defaults )
    );
